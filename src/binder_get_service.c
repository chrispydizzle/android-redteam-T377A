...
#define BR_FAILED_REPLY     _IO('r', 4)
    ...
    for (int code = 1; code <= 3; code++) {
        ...
        while (rptr < rend) {
            ...
            if (rcmd == BR_TRANSACTION_COMPLETE) {
                ...
                // Don't return -2 yet, maybe next code works?
                // Actually if we get BR_REPLY, this code was handled.
                return -2;
            } else if (rcmd == BR_DEAD_REPLY) {
                printf("    BR_DEAD_REPLY\n");
            } else if (rcmd == BR_FAILED_REPLY) {
                printf("    BR_FAILED_REPLY\n");
