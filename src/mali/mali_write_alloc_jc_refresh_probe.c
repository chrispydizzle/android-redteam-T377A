...
struct refresh_case {
    const char *name;
    uint8_t rewrite_before_retry;
    uint8_t msync_before_retry;
};
...
static void run_case(const struct refresh_case *tc)
{
    ...
    if (tc->rewrite_before_retry)
        build_zero_desc(jc1_map, alias_gpu_va);
    if (tc->msync_before_retry)
        msync(jc1_map, 4096, MS_SYNC | MS_INVALIDATE);
...
int main(void)
{
    static const struct refresh_case cases[] = {
        { "stale-jc-no-refresh", 0, 0 },
        { "rewrite-only", 1, 0 },
        { "rewrite-and-msync", 1, 1 },
        { "msync-only", 0, 1 }
    };
    size_t i;
    pid_t pid;
    ...
    printf("=== Mali write-alloc JC refresh probe ===\n");
