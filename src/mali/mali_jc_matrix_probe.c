...
static void pack_job_header(uint32_t *cl)
{
    ...
    cl[4] = (uint32_t)(pack_bits64(h.is_64b, 0) |
...
static void dump_words(const char *label, const uint32_t *buf)
{
    size_t i;
                     ...
                     size_t offset)
{
    ...
    uint32_t a1 = 0, a2 = 0;
    uint64_t target = offset == (size_t)-1 ? scratch_gpu_va : (jc2_gpu_va + offset);
    int i;
    ...
    printf("\ncase offset=%s%s target=0x%llx\n",
           offset == (size_t)-1 ? "control" : "",
           offset == (size_t)-1 ? "" : "",
           (unsigned long long)target);
    dump_words("jc2 before:", jc2_cpu);

    if (submit_atoms(fd, atoms) < 0) {
    ...
    dump_words("jc2 after: ", jc2_cpu);
    printf("dest=0x%016llx atom1=0x%08x(%s) atom2=0x%08x(%s)\n",
           (unsigned long long)dest_cpu[0],
           a1, event_name(a1), a2, event_name(a2));
}

static void run_probe(void)
{
    uint64_t atoms_gpu_va = 0, jc1_gpu_va = 0, jc2_gpu_va = 0, dest_gpu_va = 0, scratch_gpu_va = 0;
    struct base_jd_atom_v2_old56 *atoms = MAP_FAILED;
    uint32_t *jc1_cpu = MAP_FAILED, *jc2_cpu = MAP_FAILED;
    uint64_t *dest_cpu = MAP_FAILED;
    size_t offsets[] = { (size_t)-1, 0, 8, 16, 24, 32, 40, 48 };
    int fd, i;
    ...
    jc1_gpu_va = mali_alloc(fd);
    jc2_gpu_va = mali_alloc(fd);
    ...
    jc1_cpu = (uint32_t *)mali_cpu_map(fd, jc1_gpu_va);
    jc2_cpu = (uint32_t *)mali_cpu_map(fd, jc2_gpu_va);
    dest_cpu = (uint64_t *)mali_cpu_map(fd, dest_gpu_va);
    if (atoms == MAP_FAILED || jc1_cpu == MAP_FAILED || jc2_cpu == MAP_FAILED || dest_cpu == MAP_FAILED)
        goto out;

    for (i = 0; i < (int)(sizeof(offsets) / sizeof(offsets[0])); i++)
        run_case(fd, atoms, jc1_cpu, jc2_cpu, jc1_gpu_va, jc2_gpu_va, dest_gpu_va, scratch_gpu_va, dest_cpu, offsets[i]);
