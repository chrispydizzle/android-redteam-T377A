...
static void run_case(const struct ordering_case *tc)
{
    ...
    if (!alias_gpu_va || !jc1_gpu_va || !jc2_gpu_va || !jc3_gpu_va || !dest_gpu_va ||
    ...
    if (tc->middle_kind == MIDDLE_SAME_ALIAS)
        middle_target = alias_gpu_va;
    else if (tc->middle_kind == MIDDLE_ALIAS2)
    ...
    build_zero_desc(jc3_map, alias_gpu_va, tc->retry_barrier, tc->retry_invalidate);
    msync(jc3_map, 4096, MS_SYNC | MS_INVALIDATE);
    if (submit_zero_job(fd, jc3_gpu_va, 3, &ev3) < 0) {
    ...
    if (alias3_gpu_va) {
        build_zero_desc(jc1_map, alias3_gpu_va, 0, 0);
        msync(jc1_map, 4096, MS_SYNC | MS_INVALIDATE);
        if (submit_zero_job(fd, jc1_gpu_va, 4, &ev4) < 0)
            printf("fresh alias submit failed errno=%d (%s)\n", errno, strerror(errno));
    }

    printf("result first=0x%08x(%s) middle=0x%08x(%s) retry=0x%08x(%s) dest=0x%016llx fresh=0x%08x(%s)\n",
...
int main(void)
{
    static const struct ordering_case cases[] = {
        { "same-alias-control", MIDDLE_SAME_ALIAS, 0, 0, 0, 0 },
        { "native-default", MIDDLE_NATIVE_DEST, 0, 0, 0, 0 },
        { "native-middle-barrier", MIDDLE_NATIVE_DEST, 1, 0, 0, 0 },
        { "native-middle-inval", MIDDLE_NATIVE_DEST, 0, 1, 0, 0 },
        { "native-middle-barrier-inval", MIDDLE_NATIVE_DEST, 1, 1, 0, 0 },
        { "native-retry-barrier", MIDDLE_NATIVE_DEST, 0, 0, 1, 0 },
        { "native-retry-inval", MIDDLE_NATIVE_DEST, 0, 0, 0, 1 },
        { "alias2-middle", MIDDLE_ALIAS2, 0, 0, 0, 0 }
    };
    size_t i;
    pid_t pid;
    int status = 0;

    printf("=== Mali write-alloc ordering probe ===\n");
