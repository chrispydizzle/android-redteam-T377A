...
static void dump_desc(const char *label, const uint32_t *cl)
{
    int i;
...
static void run_case(int fd,
                     ...
                     uint32_t *writer_jc_cpu,
                     uint32_t *chain_cpu,
                     uint64_t writer_jc_gpu_va,
                     uint64_t chain_gpu_va,
                     uint64_t scratch_gpu_va,
                     uint64_t mid_gpu_va,
                     uint64_t dest_gpu_va,
                     uint64_t canary_gpu_va,
                     uint64_t *scratch_cpu,
                     uint64_t *mid_cpu,
                     uint64_t *dest_cpu,
                     uint64_t corrupt_target)
{
    ...
    pack_write_value_desc(writer_jc_cpu, corrupt_target, 0);
    ...
    printf("\n=== %s ===\n", label);
    dump_desc("chain desc0:", chain_cpu);
    dump_desc("chain desc1:", chain_cpu + 16);
    dump_desc("chain desc2:", chain_cpu + 32);
    ...
    msync(mid_cpu, 4096, MS_SYNC | MS_INVALIDATE);
    msync(dest_cpu, 4096, MS_SYNC | MS_INVALIDATE);
    dump_desc("chain after0:", chain_cpu);
    dump_desc("chain after1:", chain_cpu + 16);
    dump_desc("chain after2:", chain_cpu + 32);
    printf("scratch=0x%016llx mid=0x%016llx dest=0x%016llx canary_target=0x%llx atom1=0x%08x(%s) atom2=0x%08x(%s)\n",
           (unsigned long long)scratch_cpu[0],
           (unsigned long long)mid_cpu[0],
           (unsigned long long)dest_cpu[0],
           (unsigned long long)corrupt_target,
           atom1, event_name(atom1),
           atom2, event_name(atom2));
...
static void run_probe(void)
{
    ...
    uint64_t scratch_gpu_va = 0, mid_gpu_va = 0, dest_gpu_va = 0, canary_gpu_va = 0;
    ...
    canary_gpu_va = mali_alloc(fd);
    if (!atoms_gpu_va || !writer_jc_gpu_va || !chain_gpu_va || !scratch_gpu_va || !mid_gpu_va || !dest_gpu_va || !canary_gpu_va)
        goto out;
    ...
    run_case(fd, "control: 3-descriptor chain should zero scratch, mid, then dest",
             atoms, writer_jc_cpu, chain_cpu, writer_jc_gpu_va, chain_gpu_va,
             scratch_gpu_va, mid_gpu_va, dest_gpu_va, canary_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
             canary_gpu_va);

    run_case(fd, "zero desc0 next pointer before chain runs",
             atoms, writer_jc_cpu, chain_cpu, writer_jc_gpu_va, chain_gpu_va,
             scratch_gpu_va, mid_gpu_va, dest_gpu_va, canary_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
             chain_gpu_va + 24);

    run_case(fd, "zero desc1 next pointer before chain runs",
             atoms, writer_jc_cpu, chain_cpu, writer_jc_gpu_va, chain_gpu_va,
             scratch_gpu_va, mid_gpu_va, dest_gpu_va, canary_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
             chain_gpu_va + 64 + 24);

    run_case(fd, "corrupt desc2 type before chain runs",
             atoms, writer_jc_cpu, chain_cpu, writer_jc_gpu_va, chain_gpu_va,
             scratch_gpu_va, mid_gpu_va, dest_gpu_va, canary_gpu_va, scratch_cpu, mid_cpu, dest_cpu,
             chain_gpu_va + 128 + 40);
