...
struct ordering_case {
    const char *name;
    int middle_kind;
    uint8_t retry_use_jc1;
    uint8_t retry_target_kind;
    uint8_t middle_barrier;
    uint8_t middle_invalidate;
    uint8_t retry_barrier;
    uint8_t retry_invalidate;
};

enum {
    MIDDLE_NONE = 0,
    MIDDLE_NATIVE_DEST = 1,
    MIDDLE_ALIAS2 = 2,
};
...
static void build_zero_desc(uint32_t *jc_cpu, uint64_t target_gpu_va,
                            uint8_t barrier, uint8_t invalidate_cache)
{
    memset(jc_cpu, 0, 56);
    jc_cpu[4] = (1U << 0) |
                (2U << 1) |
                ((uint32_t)barrier << 8) |
                ((uint32_t)invalidate_cache << 9) |
                (1U << 16);
...
static void run_case(const struct ordering_case *tc)
{
    int fd = -1;
    uint64_t alias_gpu_va = 0, alias2_gpu_va = 0, alias3_gpu_va = 0;
    uint64_t jc1_gpu_va = 0, jc2_gpu_va = 0, jc3_gpu_va = 0, dest_gpu_va = 0;
    uint64_t middle_target = 0, retry_target = 0, retry_jc_gpu_va = 0;
    uint32_t *retry_jc_map = MAP_FAILED;
    uint32_t *jc1_map = MAP_FAILED, *jc2_map = MAP_FAILED, *jc3_map = MAP_FAILED;
    ...
    if (tc->middle_kind == MIDDLE_ALIAS2)
        alias2_gpu_va = alias_write_alloc(fd);
    if (tc->retry_target_kind == RETRY_FRESH_ALIAS)
        alias3_gpu_va = alias_write_alloc(fd);
    jc1_gpu_va = mali_alloc_page(fd);
    jc2_gpu_va = mali_alloc_page(fd);
    jc3_gpu_va = mali_alloc_page(fd);
    dest_gpu_va = mali_alloc_page(fd);
    jc1_map = (uint32_t *)mali_cpu_map(fd, jc1_gpu_va);
    jc2_map = (uint32_t *)mali_cpu_map(fd, jc2_gpu_va);
    jc3_map = (uint32_t *)mali_cpu_map(fd, jc3_gpu_va);
    dest_map = (uint64_t *)mali_cpu_map(fd, dest_gpu_va);
    printf("alias1=0x%llx alias2=0x%llx jc1=0x%llx jc2=0x%llx jc3=0x%llx dest=0x%llx\n",
           (unsigned long long)alias_gpu_va, (unsigned long long)alias2_gpu_va,
           (unsigned long long)jc1_gpu_va, (unsigned long long)jc2_gpu_va,
           (unsigned long long)jc3_gpu_va, (unsigned long long)dest_gpu_va);
    if (!alias_gpu_va ||
        (tc->middle_kind == MIDDLE_ALIAS2 && !alias2_gpu_va) ||
        (tc->retry_target_kind == RETRY_FRESH_ALIAS && !alias3_gpu_va) ||
        !jc1_gpu_va || !jc2_gpu_va || !jc3_gpu_va || !dest_gpu_va ||
        jc1_map == MAP_FAILED || jc2_map == MAP_FAILED ||
        jc3_map == MAP_FAILED || dest_map == MAP_FAILED)
        goto out;

    memset(dest_map, 0xAA, 4096);
    build_zero_desc(jc1_map, alias_gpu_va, 0, 0);
    ...
    if (tc->middle_kind == MIDDLE_ALIAS2)
        middle_target = alias2_gpu_va;
    else if (tc->middle_kind == MIDDLE_NATIVE_DEST)
        middle_target = dest_gpu_va;

    memset(&ev2, 0, sizeof(ev2));
    if (tc->middle_kind != MIDDLE_NONE) {
        build_zero_desc(jc2_map, middle_target, tc->middle_barrier, tc->middle_invalidate);
        msync(jc2_map, 4096, MS_SYNC | MS_INVALIDATE);
        if (submit_zero_job(fd, jc2_gpu_va, 2, &ev2) < 0) {
            printf("middle submit failed errno=%d (%s)\n", errno, strerror(errno));
            goto out;
        }
    }

    retry_jc_gpu_va = tc->retry_use_jc1 ? jc1_gpu_va : jc3_gpu_va;
    retry_jc_map = tc->retry_use_jc1 ? jc1_map : jc3_map;
    retry_target = (tc->retry_target_kind == RETRY_FRESH_ALIAS) ? alias3_gpu_va : alias_gpu_va;
    build_zero_desc(retry_jc_map, retry_target, tc->retry_barrier, tc->retry_invalidate);
    msync(retry_jc_map, 4096, MS_SYNC | MS_INVALIDATE);
    if (submit_zero_job(fd, retry_jc_gpu_va, 3, &ev3) < 0) {
        printf("retry submit failed errno=%d (%s)\n", errno, strerror(errno));
        goto out;
    }

    printf("result first=0x%08x(%s) middle=0x%08x(%s) retry=0x%08x(%s) dest=0x%016llx\n",
...
out:
    ...
    mali_free_checked(fd, jc3_gpu_va);
    mali_free_checked(fd, jc2_gpu_va);
    mali_free_checked(fd, jc1_gpu_va);
    mali_free_checked(fd, alias3_gpu_va);
...
int main(void)
{
    static const struct ordering_case cases[] = {
        { "back-to-back-same-jc-old-alias", MIDDLE_NONE, 1, RETRY_OLD_ALIAS, 0, 0, 0, 0 },
        { "native-middle-same-jc-old-alias", MIDDLE_NATIVE_DEST, 1, RETRY_OLD_ALIAS, 0, 0, 0, 0 },
        { "native-middle-same-jc-old-alias-barrier", MIDDLE_NATIVE_DEST, 1, RETRY_OLD_ALIAS, 0, 0, 1, 0 },
        { "native-middle-same-jc-old-alias-inval", MIDDLE_NATIVE_DEST, 1, RETRY_OLD_ALIAS, 0, 0, 0, 1 },
        { "native-middle-same-jc-fresh-alias", MIDDLE_NATIVE_DEST, 1, RETRY_FRESH_ALIAS, 0, 0, 0, 0 },
        { "native-middle-fresh-jc-old-alias", MIDDLE_NATIVE_DEST, 0, RETRY_OLD_ALIAS, 0, 0, 0, 0 },
        { "alias2-middle-same-jc-old-alias", MIDDLE_ALIAS2, 1, RETRY_OLD_ALIAS, 0, 0, 0, 0 }
    };
    size_t i;
    pid_t pid;
    int status = 0;

    printf("=== Mali write-alloc reuse probe ===\n");
