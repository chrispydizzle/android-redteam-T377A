...
static void run_case(uint32_t offset)
{
    int fd = -1;
    ...
    build_zero_desc(jc1_map, alias_gpu_va + offset);
    ...
    if (alias2_gpu_va) {
        ...
        if (submit_zero_job(fd, jc1_gpu_va, 4, &ev4) < 0)
            ...
            printf("re-alias target=0x%08x(%s)\n", ev4.event_code, event_name(ev4.event_code));
    }

    printf("result prewrite=0x%08x(%s) baseline=0x%08x(%s) dest=0x%016llx re-target=0x%08x(%s)\n",
...
int main(void)
{
    ...
    printf("=== Mali write-alloc side-effect probe ===\n");
    for (i = 0; i < sizeof(offsets) / sizeof(offsets[0]); i++) {
        ...
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
            printf("case offset=0x%03x exited abnormally status=0x%x\n", offsets[i], status);
    }
    return 0;
}
