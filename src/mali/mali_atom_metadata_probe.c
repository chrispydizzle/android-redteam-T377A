...
static int submit_atoms(int fd, struct base_jd_atom_v2_old56 *atoms, uint32_t nr_atoms)
{
    ...
    printf("JOB_SUBMIT rc=%d errno=%d (%s) ret=%u nr_atoms=%u stride=%u addr=%p\n",
           rc, errno, strerror(errno), submit.header.ret, nr_atoms,
           submit.stride, (void *)atoms);
...
static void dump_atom(const char *label, const struct base_jd_atom_v2_old56 *atom)
{
    const uint32_t *w = (const uint32_t *)atom;
    size_t i;

    printf("%s", label);
    for (i = 0; i < sizeof(*atom) / sizeof(*w); i++)
        printf(" %08x", w[i]);
    printf("\n");
}

static void init_atoms(struct base_jd_atom_v2_old56 *atoms,
                       uint64_t jc1_gpu_va,
                       uint64_t jc2_gpu_va)
{
...
static void run_case(int fd,
                     ...
                     uint64_t atoms_gpu_va,
                     uint32_t *jc1_cpu,
                     uint32_t *jc2_cpu,
                     uint64_t jc1_gpu_va,
                     uint64_t jc2_gpu_va,
                     uint64_t scratch_gpu_va,
                     uint64_t *dest_cpu,
                     uint64_t dest_gpu_va,
                     size_t atom_target_offset)
{
    struct base_jd_event_v2 ev;
    int seen[3] = { 0 };
    int i;
    uint64_t target = atom_target_offset == (size_t)-1
        ? scratch_gpu_va
        : atoms_gpu_va + atom_target_offset;

    printf("\n=== %s ===\n", label);

    init_atoms(atoms, jc1_gpu_va, jc2_gpu_va);
    build_zero_job(jc1_cpu, target);
    build_zero_job(jc2_cpu, dest_gpu_va);
    memset(dest_cpu, 0xAA, 4096);
    msync(dest_cpu, 4096, MS_SYNC | MS_INVALIDATE);

    dump_atom("atom1 before:", &atoms[0]);
    dump_atom("atom2 before:", &atoms[1]);
    printf("jc1 target=0x%llx dest before=0x%016llx\n",
...
{
    uint64_t atoms_gpu_va = 0, jc1_gpu_va = 0, jc2_gpu_va = 0;
    uint64_t dest_gpu_va = 0, scratch_gpu_va = 0;
    ...
    atoms_gpu_va = mali_alloc(fd, 1);
    jc1_gpu_va = mali_alloc(fd, 1);
    jc2_gpu_va = mali_alloc(fd, 1);
    dest_gpu_va = mali_alloc(fd, 1);
    scratch_gpu_va = mali_alloc(fd, 1);
    if (!atoms_gpu_va || !jc1_gpu_va || !jc2_gpu_va || !dest_gpu_va || !scratch_gpu_va)
        goto out;

    atoms = (struct base_jd_atom_v2_old56 *)mali_cpu_map(fd, atoms_gpu_va, 4096);
    ...
    if (atoms == MAP_FAILED || jc1_cpu == MAP_FAILED ||
        jc2_cpu == MAP_FAILED || dest_cpu == MAP_FAILED) {
        goto out;
    }

    run_case(fd, "control: atom buffer shared, atom1 zeros scratch",
             atoms, atoms_gpu_va, jc1_cpu, jc2_cpu,
             jc1_gpu_va, jc2_gpu_va, scratch_gpu_va,
             dest_cpu, dest_gpu_va, (size_t)-1);

    run_case(fd, "zero atom2 udata[0] in shared atom buffer",
             atoms, atoms_gpu_va, jc1_cpu, jc2_cpu,
             jc1_gpu_va, jc2_gpu_va, scratch_gpu_va,
             dest_cpu, dest_gpu_va, sizeof(*atoms) + 8);

    run_case(fd, "zero atom2 jc in shared atom buffer",
             atoms, atoms_gpu_va, jc1_cpu, jc2_cpu,
             jc1_gpu_va, jc2_gpu_va, scratch_gpu_va,
             dest_cpu, dest_gpu_va, sizeof(*atoms) + 0);

    run_case(fd, "zero atom2 core_req+deps qword in shared atom buffer",
             atoms, atoms_gpu_va, jc1_cpu, jc2_cpu,
             jc1_gpu_va, jc2_gpu_va, scratch_gpu_va,
             dest_cpu, dest_gpu_va, sizeof(*atoms) + 32);
