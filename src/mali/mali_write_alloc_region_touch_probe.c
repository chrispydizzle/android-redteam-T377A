...
static void run_case(const struct refresh_case *tc)
{
    ...
    case TOUCH_FIRST16:
        ...
        break;
    case TOUCH_SECOND16:
        memcpy(((uint8_t *)jc1_map) + 16, ((uint8_t *)saved_desc) + 16, 16);
        break;
    case TOUCH_FIRST32:
        memcpy(jc1_map, saved_desc, 32);
        break;
    case TOUCH_LAST24:
        memcpy(((uint8_t *)jc1_map) + 32, ((uint8_t *)saved_desc) + 32, 24);
        break;
    case TOUCH_ACTIVE28:
        memcpy(((uint8_t *)jc1_map) + 16, ((uint8_t *)saved_desc) + 16, 28);
        break;
...
int main(void)
{
    static const struct refresh_case cases[] = {
        { "stale-jc-no-touch", TOUCH_NONE },
        { "touch-first16", TOUCH_FIRST16 },
        { "touch-second16", TOUCH_SECOND16 },
        { "touch-first32", TOUCH_FIRST32 },
        { "touch-last24", TOUCH_LAST24 },
        { "touch-active28", TOUCH_ACTIVE28 },
        { "touch-full56", TOUCH_FULL_DESC }
    };
    size_t i;
    pid_t pid;
    int status = 0;

    printf("=== Mali write-alloc region-touch probe ===\n");
