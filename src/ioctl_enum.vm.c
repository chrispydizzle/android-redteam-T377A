/*
 * ioctl_enum.c — Educational ioctl enumerator for QEMU ARM kernel fuzzing VM
 *
 * Enumerates known binder and ashmem ioctls, sends them to the driver,
 * and reports which ones succeed, fail, or crash the kernel.
 *
 * This runs ONLY in the QEMU VM — it will crash the kernel on purpose.
 * That's the whole point: observe kernel behavior under malformed input.
 *
 * Build: push-to-qemu.bat ioctl_enum.c ioctl_enum
 * Run:   /bin/ioctl_enum  (in QEMU VM shell)
 */
...
static void mali_uk_call(int fd, const char *name, unsigned int func_id,
    ...
    /* Mali uses _IOWR('b', 0, <size>) — but the real driver just checks _IOC_SIZE */
    unsigned long cmd = _IOWR(0x80, 0, char[buf_size]);
...
static void test_mali(void) {
    ...
    if (fd >= 0) {
        ...
        if (sigsetjmp(jump_env, 1) != 0) {
            ...
            unsigned long cmd = _IOWR(0x80, 0, char[255]);
