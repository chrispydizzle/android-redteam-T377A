#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <fcntl.h>
#include <errno.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
#include <stdint.h>
#include <time.h>

/* Binder Definitions */
#define BINDER_WRITE_READ       _IOWR('b', 1, struct binder_write_read)
#define BINDER_VERSION          _IOWR('b', 9, struct binder_version)

struct binder_write_read {
    signed long write_size;
    signed long write_consumed;
    unsigned long write_buffer;
    signed long read_size;
    signed long read_consumed;
    unsigned long read_buffer;
};

struct binder_version {
    signed long protocol_version;
};

struct binder_transaction_data {
    union {
        uint32_t handle;
        void *ptr;
    } target;
    void *cookie;
    uint32_t code;
    uint32_t flags;
    int32_t sender_pid;
    uint32_t sender_euid;
    uint32_t data_size;
    uint32_t offsets_size;
    union {
        struct {
            const void *buffer;
            const void *offsets;
        } ptr;
        uint8_t buf8[8];
    } data;
};

struct flat_binder_object {
    uint32_t type;
    uint32_t flags;
    union {
        uint32_t binder;
        uint32_t handle;
    };
    uint32_t cookie;
};

#define BC_TRANSACTION      _IOW('c', 0, struct binder_transaction_data)
#define BC_REPLY            _IOW('c', 1, struct binder_transaction_data)
#define BC_FREE_BUFFER      _IOW('c', 3, void *)
#define BC_INCREFS          _IOW('c', 4, uint32_t)
#define BC_ACQUIRE          _IOW('c', 5, uint32_t)
#define BC_RELEASE          _IOW('c', 6, uint32_t)
#define BC_DECREFS          _IOW('c', 7, uint32_t)

#define BR_TRANSACTION      _IOR('r', 2, struct binder_transaction_data)
#define BR_REPLY            _IOR('r', 3, struct binder_transaction_data)
#define BR_DEAD_REPLY       _IO('r', 5)
#define BR_TRANSACTION_COMPLETE _IO('r', 6)
#define BR_INCREFS          _IOR('r', 7, struct binder_ptr_cookie)
#define BR_ACQUIRE          _IOR('r', 8, struct binder_ptr_cookie)
#define BR_RELEASE          _IOR('r', 9, struct binder_ptr_cookie)
#define BR_DECREFS          _IOR('r', 10, struct binder_ptr_cookie)
#define BR_ATTEMPT_ACQUIRE  _IOR('r', 11, struct binder_pri_ptr_cookie)
#define BR_NOOP             _IO('r', 12)
#define BR_SPAWN_LOOPER     _IO('r', 13)
#define BR_FINISHED         _IO('r', 14)
#define BR_DEAD_BINDER      _IOR('r', 15, void *)
#define BR_CLEAR_DEATH_NOTIFICATION_DONE _IOR('r', 16, void *)
#define BR_FAILED_REPLY     _IO('r', 17)

#define TF_ACCEPT_FDS   0x10
#define BINDER_MMAP_SIZE (1024 * 1024)

#define BINDER_TYPE_HANDLE  0x73682a85
#define BINDER_TYPE_WEAK_HANDLE 0x77682a85
#define BINDER_TYPE_FD      0x66642a85

struct binder_ptr_cookie {
    void *ptr;
    void *cookie;
};

struct binder_pri_ptr_cookie {
    int32_t priority;
    void *ptr;
    void *cookie;
};

// Interface Token: "android.os.IServiceManager"
const uint16_t interface_token[] = {
    'a','n','d','r','o','i','d','.','o','s','.','I','S','e','r','v','i','c','e','M','a','n','a','g','e','r', 0
};

void write_string16(uint8_t **ptr, const char *str) {
    uint32_t len = strlen(str);
    *((uint32_t*)*ptr) = len; *ptr += 4;
    for (uint32_t i=0; i<len; i++) {
        *((uint16_t*)*ptr) = (uint16_t)str[i]; *ptr += 2;
    }
    *((uint16_t*)*ptr) = 0; *ptr += 2; // null term
    while (((unsigned long)*ptr) % 4) *ptr += 1;
}

void write_interface_token(uint8_t **ptr) {
    *((uint32_t*)*ptr) = 0; *ptr += 4; // StrictMode policy

    uint32_t len = sizeof(interface_token)/2 - 1; 
    *((uint32_t*)*ptr) = len; *ptr += 4;
    memcpy(*ptr, interface_token, sizeof(interface_token));
    *ptr += sizeof(interface_token); 
    while (((unsigned long)*ptr) % 4) *ptr += 1;
}

void *g_binder_mmap = NULL;

int process_reply(int fd, uint8_t *rbuf, size_t rbuf_size, struct binder_write_read *bwr) {
    int got_reply = 0;
    while (!got_reply) {
        int ret = ioctl(fd, BINDER_WRITE_READ, bwr);
        if (ret < 0) {
            perror("BINDER_WRITE_READ loop");
            return -1;
        }

        if (bwr->read_consumed == 0) {
            bwr->write_size = 0;
            bwr->read_buffer = (unsigned long)rbuf;
            bwr->read_size = rbuf_size;
            bwr->read_consumed = 0;
            continue;
        }

        uint8_t *rptr = rbuf;
        uint8_t *rend = rbuf + bwr->read_consumed;

        while (rptr < rend) {
            uint32_t rcmd = *((uint32_t*)rptr); rptr += 4;
            
            if (rcmd == BR_NOOP) continue;
            if (rcmd == BR_TRANSACTION_COMPLETE) continue;
            if (rcmd == BR_SPAWN_LOOPER) continue;
            if (rcmd == BR_FINISHED) continue;
            if (rcmd == BR_CLEAR_DEATH_NOTIFICATION_DONE) { rptr += sizeof(void*); continue; }
            if (rcmd == BR_DEAD_BINDER) { rptr += sizeof(void*); continue; }

            if (rcmd == BR_REPLY) {
                struct binder_transaction_data *tr = (struct binder_transaction_data*)rptr;
                rptr += sizeof(struct binder_transaction_data);
                // Return handle if found
                if (tr->offsets_size >= 4) {
                        uint32_t *offsets = (uint32_t *)(tr->data.ptr.offsets);
                        int num_offsets = tr->offsets_size / sizeof(uint32_t);
                        uint8_t *data_ptr = (uint8_t*)(tr->data.ptr.buffer);
                        for (int i=0; i<num_offsets; i++) {
                            struct flat_binder_object *obj = (struct flat_binder_object*)(data_ptr + offsets[i]);
                            if (obj->type == BINDER_TYPE_HANDLE) return obj->handle;
                        }
                }
                got_reply = 1;
                break;
            }
            
            if (rcmd == BR_DEAD_REPLY || rcmd == BR_FAILED_REPLY) {
                got_reply = 1;
                break;
            }
            if (rcmd == BR_INCREFS || rcmd == BR_ACQUIRE || rcmd == BR_RELEASE || rcmd == BR_DECREFS) {
                rptr += sizeof(struct binder_ptr_cookie);
            } else if (rcmd == BR_ATTEMPT_ACQUIRE) {
                rptr += sizeof(struct binder_pri_ptr_cookie);
            } else {
                    // Unknown
                    got_reply = 1; 
                    break;
            }
        }
        if (got_reply) break;
        
        bwr->write_size = 0;
        bwr->read_buffer = (unsigned long)rbuf;
        bwr->read_size = rbuf_size;
        bwr->read_consumed = 0;
    }
    return -2; // Reply but no handle
}


int get_service_handle(int fd, const char *service_name) {
    uint8_t wbuf[1024];
    uint8_t rbuf[1024];
    struct binder_write_read bwr;
    memset(&bwr, 0, sizeof(bwr));

    uint8_t tdata[512];
    uint8_t *ptr = tdata;
    struct binder_transaction_data td;

    // Use code 1 (GET_SERVICE) as verified
    int code = 1;
    memset(&bwr, 0, sizeof(bwr));
    ptr = tdata;
    write_interface_token(&ptr);
    write_string16(&ptr, service_name);

    memset(&td, 0, sizeof(td));
    td.target.handle = 0; 
    td.code = code; 
    td.flags = 0; 
    td.data_size = ptr - tdata;
    td.data.ptr.buffer = tdata;
    td.offsets_size = 0;

    uint32_t cmd = BC_TRANSACTION;
    uint8_t *wptr = wbuf;
    memcpy(wptr, &cmd, 4); wptr += 4;
    memcpy(wptr, &td, sizeof(td)); wptr += sizeof(td);

    bwr.write_buffer = (unsigned long)wbuf;
    bwr.write_size = wptr - wbuf;
    bwr.read_buffer = (unsigned long)rbuf;
    bwr.read_size = sizeof(rbuf);

    return process_reply(fd, rbuf, sizeof(rbuf), &bwr);
}

void fuzz_service(int fd, int handle, const char *service_name) {
    uint8_t wbuf[1024];
    uint8_t rbuf[1024];
    struct binder_write_read bwr;
    struct binder_transaction_data td;
    uint32_t tdata_aligned[256]; 
    uint8_t *tdata = (uint8_t*)tdata_aligned;

    printf("[*] Fuzzing service handle %d ('%s')...\n", handle, service_name);

    // Increase refcount on handle
    {
        uint32_t cmd = BC_ACQUIRE;
        uint32_t target = handle;
        uint8_t *wptr = wbuf;
        memcpy(wptr, &cmd, 4); wptr += 4;
        memcpy(wptr, &target, 4); wptr += 4;
        
        bwr.write_buffer = (unsigned long)wbuf;
        bwr.write_size = wptr - wbuf;
        bwr.read_size = 0;
        ioctl(fd, BINDER_WRITE_READ, &bwr);
    }

    srand(time(NULL));
    
    // Guess interface token based on service name
    char interface_token_str[128];
    memset(interface_token_str, 0, sizeof(interface_token_str));
    
    if (strcmp(service_name, "activity") == 0) {
        strcpy(interface_token_str, "android.app.IActivityManager");
    } else if (strcmp(service_name, "package") == 0) {
        strcpy(interface_token_str, "android.content.pm.IPackageManager");
    } else if (strcmp(service_name, "window") == 0) {
        strcpy(interface_token_str, "android.view.IWindowManager");
    } else if (strcmp(service_name, "alarm") == 0) {
        strcpy(interface_token_str, "android.app.IAlarmManager");
    } else if (strcmp(service_name, "power") == 0) {
        strcpy(interface_token_str, "android.os.IPowerManager");
    } else if (strcmp(service_name, "media.player") == 0) {
        strcpy(interface_token_str, "android.media.IMediaPlayerService");
    } else if (strcmp(service_name, "SurfaceFlinger") == 0) {
        strcpy(interface_token_str, "android.ui.ISurfaceComposer");
    } else if (strcmp(service_name, "drm.drmManager") == 0) {
        strcpy(interface_token_str, "drm.IDrmManagerService");
    } else if (strcmp(service_name, "media.camera") == 0) {
        strcpy(interface_token_str, "android.hardware.ICameraService");
    } else {
        // Fallback guess: I + CapitalizedService
        if (strlen(service_name) > 0) {
            sprintf(interface_token_str, "android.os.I%c%s", 
                (service_name[0] >= 'a' && service_name[0] <= 'z') ? service_name[0]-32 : service_name[0], 
                service_name+1);
        } else {
            strcpy(interface_token_str, "android.os.IBinder");
        }
        printf("[?] Guessing interface token: %s\n", interface_token_str);
    }

    // Try transaction codes 1-200
    for (int code = 1; code <= 200; code++) {
        // if (code % 10 == 0) printf("    Testing code %d...\n", code);

        memset(&td, 0, sizeof(td));
        td.target.handle = handle;
        td.code = code;
        td.flags = 0; 

        // Construct payload
        uint8_t *ptr = tdata;
        
        // 1. StrictMode Policy (int32) - 0
        *((uint32_t*)ptr) = 0; ptr += 4;

        // 2. Interface Token Length
        uint32_t len = strlen(interface_token_str);
        *((uint32_t*)ptr) = len; ptr += 4;
        
        // 3. Interface Token Chars (16-bit)
        for (uint32_t i=0; i<len; i++) {
            *((uint16_t*)ptr) = (uint16_t)interface_token_str[i]; ptr += 2;
        }
        *((uint16_t*)ptr) = 0; ptr += 2; // null term
        while (((unsigned long)ptr) % 4) ptr += 1; // align

        // 4. Random Data
        int payload_len = rand() % 256;
        // Ensure we don't overflow tdata_aligned (1024 bytes)
        // Current ptr is at most 4 + 4 + 128*2 + 4 = ~270 bytes.
        // So plenty of space.
        
        for (int i=0; i<payload_len; i++) {
            *ptr = rand() % 256; ptr++;
        }
        
        td.data_size = ptr - tdata;
        td.data.ptr.buffer = tdata;
        td.offsets_size = 0;

        uint32_t cmd = BC_TRANSACTION;
        uint8_t *wptr = wbuf;
        memcpy(wptr, &cmd, 4); wptr += 4;
        memcpy(wptr, &td, sizeof(td)); wptr += sizeof(td);

        bwr.write_buffer = (unsigned long)wbuf;
        bwr.write_size = wptr - wbuf;
        bwr.read_buffer = (unsigned long)rbuf;
        bwr.read_size = sizeof(rbuf);
        bwr.read_consumed = 0;

        // Send and wait for reply
        int ret = ioctl(fd, BINDER_WRITE_READ, &bwr);
        if (ret >= 0) {
             process_reply(fd, rbuf, sizeof(rbuf), &bwr);
        }
    }
}

int main(int argc, char **argv) {
    if (argc < 2) {
        printf("Usage: %s <service_name>\n", argv[0]);
        return 0;
    }

    int fd = open("/dev/binder", O_RDWR);
    if (fd < 0) { perror("open"); return 1; }
    
    g_binder_mmap = mmap(NULL, BINDER_MMAP_SIZE, PROT_READ, MAP_PRIVATE, fd, 0);
    if (g_binder_mmap == MAP_FAILED) { perror("mmap"); return 1; }

    struct binder_version ver;
    ioctl(fd, BINDER_VERSION, &ver);

    printf("[*] Resolving service '%s'...\n", argv[1]);
    int handle = get_service_handle(fd, argv[1]);
    
    if (handle >= 0) {
        printf("[+] Found service '%s' -> Handle %d\n", argv[1], handle);
        fuzz_service(fd, handle, argv[1]);
    } else {
        printf("[-] Failed to get service '%s' (err=%d)\n", argv[1], handle);
    }
    return 0;
}
