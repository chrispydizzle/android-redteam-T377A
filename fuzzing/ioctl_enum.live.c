 ...
 *
 * CRITICAL: The real driver does strict size checking:
 *   if (sizeof(real_struct) != args_size) → reject
 * Sizes below are from Samsung GPL source (drivers/gpu/arm/t72x/r7p0/).
 */
...
/* sizeof = 80 — NOT the same as uku_version_check_args! */
...
/* sizeof = 32 (with Samsung SYSTRACE addition) */
struct kbase_uk_job_submit {
    union uk_header header;
    uint64_t addr_sizer;  /* union kbase_pointer (8 bytes) */
    uint32_t nr_atoms;
    uint32_t stride;
    uint32_t gles_ctx_handle;  /* Samsung SYSTRACE addition */
    uint32_t padding;
};

/* sizeof = 536 — GPU property tree (matches original CALL_MAX_SIZE) */
#define KBASE_UK_GPUPROPS_SIZE 536

/* Mali base memory flags (from mali_base_kernel.h) */
...
static void test_mali(void) {
    ...
    printf("    (struct sizes from Samsung GPL source)\n\n");
    ...
    /* Step 1: Version handshake — uses uku_version_check_args (16 bytes) */
    {
    ...
    /* Step 2: SET_FLAGS — kbase_uk_set_flags (16 bytes) */
    {
        struct kbase_uk_set_flags flags;
        memset(&flags, 0, sizeof(flags));
        flags.create_flags = 0;  /* BASE_CONTEXT_CREATE_FLAG_NONE */
        printf("    [sizeof=%zu]\n", sizeof(flags));
        mali_uk_call(fd, "SET_FLAGS", KBASE_FUNC_SET_FLAGS,
                     &flags, sizeof(flags));
    }

    /* Step 3: GET_VERSION — kbase_uk_get_ddk_version (80 bytes, NOT 16!) */
    {
        struct kbase_uk_get_ddk_version ver;
        memset(&ver, 0, sizeof(ver));
        printf("    [sizeof=%zu]\n", sizeof(ver));
        mali_uk_call(fd, "GET_VERSION", KBASE_FUNC_GET_VERSION,
                     &ver, sizeof(ver));
        printf("    -> DDK version: \"%.*s\" (len=%u)\n",
               (int)ver.version_string_size, ver.version_buffer,
               ver.version_string_size);
    }

    /* GET_CONTEXT_ID — kbase_uk_context_id (16 bytes) */
    {
    ...
    /* DISJOINT_QUERY — kbase_uk_disjoint_query (16 bytes) */
    {
    ...
    /* MEM_ALLOC — kbase_uk_mem_alloc (56 bytes) */
    {
        struct kbase_uk_mem_alloc mem;
        memset(&mem, 0, sizeof(mem));
        mem.va_pages = 16;
        mem.commit_pages = 16;
        /* Need valid flags or driver rejects with "bad flags" */
        mem.flags = BASE_MEM_PROT_CPU_RD | BASE_MEM_PROT_CPU_WR |
                    BASE_MEM_PROT_GPU_RD | BASE_MEM_PROT_GPU_WR |
                    BASE_MEM_SAME_VA;
        printf("    [sizeof=%zu, flags=0x%llx]\n", sizeof(mem), mem.flags);
        mali_uk_call(fd, "MEM_ALLOC(16 pages)", KBASE_FUNC_MEM_ALLOC,
                     &mem, sizeof(mem));
        printf("    -> gpu_va: 0x%llx\n", (unsigned long long)mem.gpu_va);

        if (mem.gpu_va != 0) {
            /* MEM_QUERY on the allocation */
            struct kbase_uk_mem_query q;
            memset(&q, 0, sizeof(q));
            q.gpu_addr = mem.gpu_va;
            q.query = 1; /* KBASE_MEM_QUERY_COMMIT_SIZE */
            mali_uk_call(fd, "MEM_QUERY(commit)", KBASE_FUNC_MEM_QUERY,
                         &q, sizeof(q));
            printf("    -> commit_size: %llu pages\n", (unsigned long long)q.value);
            ...
            printf("    (skipping QUERY/FREE — alloc failed)\n");
        }
    }

    /* GPU_PROPS_REG_DUMP — kbase_uk_gpuprops (592 bytes!) */
    {
        uint8_t gpuprops[KBASE_UK_GPUPROPS_SIZE];
        memset(gpuprops, 0, sizeof(gpuprops));
        printf("    [sizeof=%zu]\n", sizeof(gpuprops));
        mali_uk_call(fd, "GPU_PROPS_REG_DUMP", KBASE_FUNC_GPU_PROPS_REG_DUMP,
                     gpuprops, sizeof(gpuprops));
        /* First 8 bytes are uk_header, rest is mali_base_gpu_props */
        union uk_header *hdr = (union uk_header *)gpuprops;
        if (hdr->ret == 0) {
            /* product_id is at offset 8 (first field of core_props) */
            uint32_t product_id;
            memcpy(&product_id, &gpuprops[8], 4);
            printf("    -> gpu_product_id: 0x%04x\n", product_id);
        }
    }

    /* HWCNT_SETUP — kbase_uk_hwcnt_setup (40 bytes)
     * NOTE: Requires KBASE_SEC_INSTR_HW_COUNTERS_COLLECT capability.
     * Will return EFAULT (via out_bad → -EINVAL) if permission denied. */
    {
        struct kbase_uk_hwcnt_setup hwcnt;
        memset(&hwcnt, 0, sizeof(hwcnt));
        printf("    [sizeof=%zu]\n", sizeof(hwcnt));
        mali_uk_call(fd, "HWCNT_SETUP", KBASE_FUNC_HWCNT_SETUP,
                     &hwcnt, sizeof(hwcnt));
    }

    /* JOB_SUBMIT — kbase_uk_job_submit (32 bytes, NOT 64) */
    {
        struct kbase_uk_job_submit job;
    ...
    /* Call before version handshake on new fd */
    close(fd);
    fd = open("/dev/mali0", O_RDWR);
    if (fd >= 0) {
        struct kbase_uk_mem_alloc mem;
    ...
    if (fd >= 0) {
        ...
        if (sigsetjmp(jump_env, 1) != 0) {
            ...
            signal(SIGSEGV, signal_handler);
            signal(SIGBUS, signal_handler);
            int ret = ioctl(fd, mali_cmd(16), NULL);
            signal(SIGSEGV, SIG_DFL);
            signal(SIGBUS, SIG_DFL);
        ...
        /* Oversized payload (> CALL_MAX_SIZE 536) → ENOTTY */
        {
            errno = 0;
            int ret = ioctl(fd, mali_cmd(600), NULL);
            printf("  [-] %-30s  ret=%d errno=%d (%s)\n",
                   "IOCTL(size=600)", ret, errno, strerror(errno));
        }
